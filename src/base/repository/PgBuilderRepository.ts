...
class PgBuilderRepository {
  ...
  protected insert(tableName: string, insertArg: Record<string, any>, returning = "*"): this {
    ...
    const values = _.values(insertArg).map((val) =>
      typeof val === "string"
        ? val.replace(new RegExp(/'/, "gmi"), "’")
  ...
  protected update(tableName: string, id: string, updateArg: Record<string, any>, returning = "*"): this {
    ...
    const values = _.values(updateArg).map((val) =>
      typeof val === "string"
        ? val.replace(new RegExp(/'/, "gmi"), "’")
        : typeof val === "object" && !Array.isArray(val)
        ? JSON.stringify(val)
    ...
    for (let i = 0; i < keys.length; i++) {
      this.updateQuery += `\n\t${this.replaceChar} = ${
        typeof values[i] === "string"
          ? `'${this.replaceChar}'`
          : typeof values[i] === "object" && Array.isArray(values[i])
          ? `'{${this.replaceChar}}'`
          : this.replaceChar
      }${i === keys.length - 1 ? "" : ","}`
      this.updateParams.push(keys[i])
      this.updateParams.push(
        typeof values[i] === "object" && !Array.isArray(values[i]) ? JSON.stringify(values[i]) : values[i]
      )
    }
  ...
  protected upsert_query(tableName: string, args: Record<string, any>, returning = "*"): this {
    ...
    if (id || id === 0) return this.update(tableName, id, sanitizedArgs, returning)
  ...
  protected select(selectArg?: string | string[]): this {
    switch (typeof selectArg) {
      case "string":
        this.selectQuery = /* SQL */ `SELECT ${this.replaceChar}`
        this.selectParams.push(selectArg)
        break

      case "object":
        this.selectQuery = /* SQL */ `SELECT ${selectArg.map(() => `${this.replaceChar}`).join(", ")}`
      ...
      await postgresPool.pool
        ...
        .then((result) => {
          if (doNotReturn) return resolve({ rowCount: result.rowCount, rows: [] })
            ...
            const rows: Record<string, any>[] = result.rows.map((row) => _.omit(row, omits))
